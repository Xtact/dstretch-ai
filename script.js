document.addEventListener('DOMContentLoaded', () => {
    ...
    const initialize = () => {
        ...
        // Method 1: Direct click on image
        imageDisplay.addEventListener('click', function(e) {
            console.log('=== IMAGE DISPLAY CLICKED ===');
            e.preventDefault();
            e.stopPropagation();
            imageLoader.click();
        }, true);
        
        // Method 2: Click on container
        const container = document.querySelector('.image-container');
        if (container) {
            container.addEventListener('click', function(e) {
                console.log('=== IMAGE CONTAINER CLICKED ===');
                if (e.target === this || e.target === imageDisplay) {
                    e.preventDefault();
                    e.stopPropagation();
                    imageLoader.click();
                }
    ...
    function applyZoom() {
        ...
        if (zoomLevel > 1) {
            imageDisplay.classList.add('zoomed');
        } else {
            imageDisplay.classList.remove('zoomed');
        }
    }
    ...
    function startPan(e) {
        ...
        // Don't pan if clicking zoom buttons
